...
void gimbal_task(void const *pvParameters)
{
  ...
  {
    /*设置云台模式 初始化   校准    遥控
         * 并且为每种模式下YAW 和 PITCH轴设置电机参数设置方式（电流/陀螺仪/编码器）
         */
...
static void gimbal_feedback_update(gimbal_control_t *feedback_update)
{
  ...
  //云台数据更新
  /*
    feedback_update 为在初始化时设置的各编码器及数据的总指针，实时值通过中断+DMA实时反馈
    */
...
static void gimbal_mode_change_control_transit(gimbal_control_t *gimbal_mode_change)
{
  ...
  {
    /*从其他模式切换到电流模式  直接设置raw_cmd_current/目标电流为给予电流*/
...
static void gimbal_absolute_angle_limit(gimbal_motor_t *gimbal_motor, fp32 add)
{
  ...
  //now angle error
  //当前控制误差角度
  bias_angle = rad_format(gimbal_motor->absolute_angle_set /*目标角度*/ - gimbal_motor->absolute_angle /*当前角度*/);
  ...
  /*此时得到在角度范围内所需添加的角度add*/

  /*下两行将gimbal_motor->absolute_angle_set更新为目标角度*/
